-->> Problem Statement:
-- Write a query to fetch the record of brand whose amount is 
...
with cte as (
		select *,(case when amount < 
			lead(amount,1, amount+1) over(partition by brand order by year) then 1 else 0 end) as flag
		from brands )
